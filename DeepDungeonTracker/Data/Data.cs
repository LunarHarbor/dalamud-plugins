...
public sealed class Data : IDisposable
{
    ...
    public void Update(Configuration configuration)
    {
        ...
        {
            if (!this.BetweenAreas.IsActivated && !this.Occupied33.IsActivated)
            {
                this.Common.EnsureStarted(this.Text);
                this.Common.CheckForFloorChange();
                this.Common.Checkpoint();
            }
            this.CheckForSolo();
        ...
        if (!this.Common.IsLastFloor)
        {
            ...
            foreach (InventoryEventArgs e in inventoryEventArgs ?? [])
            {
                if (e.Type is GameInventoryEvent.Added or GameInventoryEvent.Changed)
                {
                    ...
                    break; // One opened coffer, even when inventory emits several changes.
                }
            }
        }
    }
