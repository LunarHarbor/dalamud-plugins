...
public unsafe class DataText
{
    ...
    public bool IsPilgrimsTraverseRegion(uint territoryType, bool checkForSubRegion = false)
    {
        if (territoryType is >= 1281 and <= 1290) return true;
        return territoryType == 816 && (!checkForSubRegion ||
            Service.GameGui.GetAddonByName("DeepDungeonSaveData", 1).Address != nint.Zero);
    }
