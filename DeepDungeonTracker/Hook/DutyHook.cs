...
namespace DeepDungeonTracker.Hook
{
    ...
    public sealed unsafe class DutyHook : IDisposable
    {
        ...
        {
            var result = this._dutyHookDelegate!.Original(a1, a2, a3);
            try
            {
                if (a3 != null && Service.Condition[ConditionFlag.InDeepDungeon] &&
                    *a3 == 0x6D && *(uint*)(a3 + 4) == 0x4000_000E)
                    NewFloorEvents.Publish();
            }
            catch (Exception e) { CaptureDiagnostics.Report("Floor notification failed", e); }
            return result;
        }
    }
}
