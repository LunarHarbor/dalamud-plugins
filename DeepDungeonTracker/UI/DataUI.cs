...
public sealed class DataUI(IUiBuilder uiBuilder) : IDisposable
{
    ...
    public bool CommonWindowVisibility(bool show, bool showInBetweenFloors, bool isInDeepDungeonRegion, bool isInsideDeepDungeon, bool isInDeepDungeonSubArea)
    {
        return this.ShowUI && show && isInDeepDungeonRegion && (isInsideDeepDungeon || (!isInsideDeepDungeon && isInDeepDungeonSubArea)) &&
            ...
            ServiceUtility.IsSupportedParty;
    }
    ...
    public void DrawDeepDungeon(float x, float y, DeepDungeon deepDungeon, Alignment align = Alignment.Left)
    {
        if (deepDungeon == DeepDungeon.PilgrimsTraverse)
        {
            var left = x - (align == Alignment.Center ? 202.5f : align == Alignment.Right ? 405f : 0f);
            var top = y - (align == Alignment.Center ? 14.625f : align == Alignment.Right ? 29.25f : 0f);
            this.Render.DrawUIElement(this.ResourceUI.DeepDungeon, left, top, 0.75f, 0, 1, 1,
                new(0f, 0f), new(125f, 39f));
            this.Render.DrawUIElement(this.ResourceUI.DeepDungeon, left + 311.25f, top, 0.75f, 0, 1, 1,
                new(415f, 0f), new(125f, 39f));
            this.DrawTextMiedingerMid(left + 202.5f, top + 14.625f, "Pilgrim's Traverse",
                new(0.91f, 0.84f, 0.66f, 1f), Alignment.Center);
        }
        else if (deepDungeon != DeepDungeon.None)
            this.Render.DrawUIElement(this.ResourceUI.DeepDungeon, x, y, 0.75f, (int)deepDungeon - 1, 1, 3, align: align);
    }
    ...
    public void DrawCoffer(float x, float y, Coffer coffer) => this.DrawItem(x, y, (int)coffer);

    private void DrawItem(float x, float y, int id)
    {
        uint icon = id switch { 22 => 27923, 23 => 27924, 24 => 27925, 67 => 27975, _ => 0 };
        if (icon == 0)
            this.Render.DrawUIElement(this.ResourceUI.Coffer, x, y, 0.5f, id, 10, 7);
        else
        {
            var texture = Service.TextureProvider.GetFromGameIcon(new(icon)).GetWrapOrDefault();
            if (texture != null)
                this.Render.DrawUIElement(texture, x, y, 32f / texture.Width, 0, 1, 1);
        }
    }
    ...
    public void DrawTrap(float x, float y, Trap trap) => this.Render.DrawUIElement(this.ResourceUI.Trap, x, y, 0.5f, (int)(trap == Trap.Fae ? Trap.Luring : trap), 3, 3, null, null, Color.Red);
    ...
    public void DrawPomander(float x, float y, Pomander pomander) => this.DrawItem(x, y, (int)pomander);
