...
public sealed class ScoreWindow(string id, Configuration configuration, Data data) : WindowEx(id, configuration), IDisposable
{
    ...
    private void ScoreFlyText()
    {
        ...
        if (this.Data.Common.Score?.Breakdown != null && config.IsFlyTextScoreVisible && this.Data.Common.EnableFlyTextScore && scoreDifference != 0)
    ...
    private Vector2 DrawDefault(Configuration.ScoreTab config, DataUI ui)
    {
        ...
        {
            y += 20.0f;
            ui.DrawTextMiedingerMid(width / 2.0f, y, "Estimate", Color.White, Alignment.Center);
        ...
        if (this.Data.Common.Score?.Breakdown != null)
            ui.DrawNumber(x, y, this.GradualScore, true, config.TotalScoreColor, Alignment.Right);
        else ui.DrawTextMiedingerMidLarge(x, y, "—", config.TotalScoreColor, Alignment.Right);
    ...
    private Vector2 DrawTextAxis(Configuration.ScoreTab config, DataUI ui, float textScale)
    {
        ...
        ui.DrawTextAxis(x / textScale, y / textScale, this.Data.Common.Score?.Breakdown != null ? this.GradualScore.ToString("N0", CultureInfo.InvariantCulture) : "—", config.TotalScoreColor, Alignment.Right);
        ui.Scale = scale;
    ...
    private Vector2 DrawTextMiedingerMid(Configuration.ScoreTab config, DataUI ui, float textScale)
    {
        ...
        var scale = ui.Scale;
        ui.Scale = scale * textScale;
        ui.DrawTextMiedingerMidLarge(x / textScale, y / textScale, this.Data.Common.Score?.Breakdown != null ? this.GradualScore.ToString("N0", CultureInfo.InvariantCulture) : "—", config.TotalScoreColor, Alignment.Right);
        ui.Scale = scale;
