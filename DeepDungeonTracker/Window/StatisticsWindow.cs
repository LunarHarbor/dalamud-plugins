...
public sealed class StatisticsWindow : WindowEx, IDisposable
{
        ...
        foreach (var item in data ?? [])
        {
            ...
            if ((!includeMapTotal && value != Miscellaneous.Map) || includeMapTotal)
            {
                if (value == Miscellaneous.RegenPotion)
                {
                    ...
                    else if (this.Data.Statistics.DeepDungeon is DeepDungeon.EurekaOrthos or DeepDungeon.PilgrimsTraverse)
    ...
    private void SummaryPageRight(float top)
    {
        ...
        if (score != null)
        {
            ...
            DrawSingle(ui, x, ref y, "Maps", score.MapScore);
            DrawSingle(ui, x, ref y, "Coffers", score.CofferScore);
            if (statistics?.SaveSlot?.DeepDungeon == DeepDungeon.PalaceOfTheDead)
                DrawSingle(ui, x, ref y, "NPCs", score.NPCScore);
            if (statistics?.SaveSlot?.DeepDungeon == DeepDungeon.EurekaOrthos)
                DrawSingle(ui, x, ref y, "Dread Beasts", score.DreadBeastScore);
            DrawSingle(ui, x, ref y, "Mimicgoras", score.MimicgoraScore);
            DrawSingle(ui, x, ref y, "Enchantments", score.EnchantmentScore);
            DrawSingle(ui, x, ref y, "Traps", score.TrapScore);
            DrawSingle(ui, x, ref y, "Time Bonuses", score.TimeBonusScore);
            DrawSingle(ui, x, ref y, "Deaths", score.DeathScore);
            y += lineHeight;

            DrawSingle(ui, x, ref y, "Clear Bonuses", (score.Breakdown?.Party ?? 0) + (score.Breakdown?.Bosses ?? 0) + (score.Breakdown?.Completion ?? 0));
            DrawSingle(ui, x, ref y, "Non-Kills", score.NonKillScore);
            DrawSingle(ui, x, ref y, "Kills", score.KillScore);
            y += lineHeight;
